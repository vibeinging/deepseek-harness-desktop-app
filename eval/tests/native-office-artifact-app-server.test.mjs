...
test('the pinned app-server accepts native office inspect, create and edit tool contracts', { timeout: 30_000 }, async (t) => {
  ...
  const runtimeHome = await mkdtemp(join(tmpdir(), 'dsh-native-office-artifact-'));
  const names = new Set(['artifact_office_inspect', 'artifact_office_create', 'artifact_office_edit']);
  ...
  try {
    const started = await kernel.startThread({
      ...
      developerInstructions: 'Inspect a current office artifact before applying stable-anchor edits.',
    });
